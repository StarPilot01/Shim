...
GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, ALTER, INDEX, REFERENCES ON ShimRoom.* TO 'shim_app'@'localhost';
FLUSH PRIVILEGES;
EXIT;
```
...
export MYSQL_DATABASE=ShimRoom
...
Copy `deploy/ShimRoom.service.example` to `/etc/systemd/system/ShimRoom.service`, then replace the user, path, and MySQL password.

```bash
sudo systemctl daemon-reload
sudo systemctl enable --now ShimRoom
sudo systemctl status ShimRoom
```
...
```text
UBUNTU_APP_DIR=/home/chan/ShimRoom
UBUNTU_SERVICE_NAME=ShimRoom
```

With a self-hosted runner on the Ubuntu server, the deploy workflow runs locally on that server, copies the repository contents into `UBUNTU_APP_DIR`, runs `npm ci --omit=dev`, and restarts `ShimRoom`.

The deploy user must be able to restart only this service without an interactive password. One narrow sudoers rule is:

```text
chan ALL=NOPASSWD: /usr/bin/systemctl restart ShimRoom
```
